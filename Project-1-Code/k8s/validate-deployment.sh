...
if kubectl -n $NAMESPACE get ingress routeclouds-ingress > /dev/null 2>&1; then
  echo -e "${GREEN}✓ Ingress exists${NC}"
  ALB_ENDPOINT=$(kubectl -n $NAMESPACE get ingress routeclouds-ingress -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
